...
year = '2018'
allgames = readcsv(year+'SKED.txt')
schedule = {}
for i in allgames:
	if i[0].replace('"','') not in schedule.keys():
		schedule[i[0].replace('"','')]=[{'vteam':i[3].replace('"',''),'hteam':i[6].replace('"',''),'vruns':-1,'hruns':-1}]
	else:
		schedule[i[0].replace('"','')].append({'vteam':i[3].replace('"',''),'hteam':i[6].replace('"',''),'vruns':-1,'hruns':-1})
print(schedule)
